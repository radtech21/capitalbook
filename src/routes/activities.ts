...
// GET /api/activity -> recent activity across all contacts (team feed)
...
activitiesRouter.get('/contacts/:id/activities', requireAuth, async (req, res) => {
  const rows = await q('SELECT id, contact_id, user_id, user_email, type, body, created_at FROM activities WHERE contact_id = ? ORDER BY id DESC', [Number(req.params.id)]);
  return res.json({ activities: rows });
});
