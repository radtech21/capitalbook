...
UPDATE contacts SET
  ...
  data_flags = TRIM(CONCAT(data_flags, ' | ',
    'CORRECTION: the batch-20 "likely moved to Sun Life" flag was wrong. A QA re-check found four independent current sources (his own practice site, LinkedIn, Facebook business page, and a June 2025 Wealth Professional Excellence Award) all showing him still at IPC Securities / Suthern Private Wealth Management, Calgary — the original single source was misread.'))
WHERE id = 1661;

-- 1672 Ross Campbell — confidence upgrade only (Q Wealth move corroborated by a second source).
UPDATE contacts SET
  data_flags = TRIM(CONCAT(data_flags, ' | QA re-check: the Q Wealth partnership is now corroborated by Camaco Capital''s own client-relationship disclosure, independent of the original email-domain inference.'))
WHERE id = 1672;

-- 1835 Antoine Niding — confidence upgrade only (RBC move corroborated by a second source).
UPDATE contacts SET
  data_flags = TRIM(CONCAT(data_flags, ' | QA re-check: the RBC Dominion Securities move is now corroborated by RBC''s own advisor directory and by his former Richardson Wealth team page being renamed to drop his name.'))
WHERE id = 1835;
...
UPDATE contacts SET
  ...
  data_flags = TRIM(CONCAT(data_flags, ' | ',
    'QA re-check: now confirmed — team''s publicly reported AUM range ($2.2B-$2.9B across sources) closely matches the CRM''s recorded $2.86B figure, corroborating the name/location match. Same NBF flagship building/suite as several other independent teams identified elsewhere in this project.'))
WHERE id = 1856;
